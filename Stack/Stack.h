    ...
    for (char current : input) {
        if (current == ' ') {
            ...
            if (!current_number.empty()) {
                ...
                value[letter_index] = std::stod(current_number); // 将数字存储到数组
                old_tokens.push(std::string(1, letter[letter_index])); // 用字母替换数字
                letter_index++;
                current_number = "";
                has_dot = false;
            }
...
void get_result(std::string input) {
    ...
    Stack<char> op_stack(len);   // 操作符栈
    Stack<char> postfix_tokens(len); // 后缀表达式栈
